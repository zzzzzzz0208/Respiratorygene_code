...
##
dir = "D:/药物/CMap_result/GLYCOLYSIS"
...
write.table(freq,"D:/药物/GLYCOLYSIS/ES大于95/每种药物在多少种癌症中相关.txt",row.names=F,sep="\t",quote=F)
...
colnames(freq)[1]<-"Drug"
freq<-as.data.frame(freq[order(freq[,2],decreasing = T),]) 
write.table(freq,"D:/药物/GLYCOLYSIS/ES小于-95/每种药物在多少种癌症中相关.txt",row.names=F,sep="\t",quote=F)
...
Fig.S6 <- ggplot(merge_pos,aes(cancer,Name,color=abs(Score)))+
...
Fig.S6 <- ggplot(merge_neg,aes(cancer,Name,color=abs(Score)))+
  geom_point(size=2.5)+   #shape=21,colour = "white",stroke = 1.5
  xlab(NULL)+ ylab(NULL)+
  theme(panel.grid.major=element_line(colour='white'))+ 
  theme(axis.text.x=element_text(color = "#424242", size=12, angle=45, hjust=1))+ 
