...
for(i in 1:nrow(neg_freq)){
  ...
  setwd("D:/KIRC-与每个通路相关的gene/neg")
  name<-paste(neg_freq[i,1],".txt")
  write.table(path,name,sep="\t",row.names = F,col.names=T,quote=F)
}

##整理画图数据
dat<-read.table('D:/Code/Data/FigS4/KIRC-与每个通路相关的差异表达基因表达谱.csv',header=T,sep=',')
datnew <- dat[,-2]
...
group2 = data.frame(Path = factor(rep(c("HALLMARK_ADIPOGENESIS","HALLMARK_BILE_ACID_METABOLISM",
                                        "HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION ","HALLMARK_FATTY_ACID_METABOLISM",
                                        "HALLMARK_IL6_JAK_STAT3_SIGNALING","HALLMARK_KRAS_SIGNALING_DN",
                                        "HALLMARK_OXIDATIVE_PHOSPHORYLATION","HALLMARK_XENOBIOTIC_METABOLISM"), 
                                      c(2,2,1,2,1,1,2,1))))
ann_colors = list(Path = c(HALLMARK_ADIPOGENESIS="#2980B9", 
                           HALLMARK_BILE_ACID_METABOLISM="#F48FB1",
                           HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION="#FBC02D", 
                           HALLMARK_FATTY_ACID_METABOLISM="#FF8F00",
                           HALLMARK_IL6_JAK_STAT3_SIGNALING="#26C6DA",
                           HALLMARK_KRAS_SIGNALING_DN="#A3E4D7",
                           HALLMARK_OXIDATIVE_PHOSPHORYLATION="#D1C4E9",
                           HALLMARK_XENOBIOTIC_METABOLISM="#A9CCE3"),                 
                  Sample = c(cancer="#EF5350", normal="#00897B"))
#
x <- replicate(12,rnorm(100,0,1),simplify = FALSE)
x <- setNames(x,paste0("A",seq_along(x)))
rownames(group2)<- names(x)
rownames(res2)<-names(x)

library(pheatmap)
Fig.S4A <- pheatmap(res2,
                    cluster_rows = F,   
                    cluster_cols = F,               
...
exp<-read.table("D:/KIRC-gene_expression.csv",header=T,row.names=1,sep=",")
...
tres<-t(res)
write.csv(tres,"D:/VCAN高低风险组分组/KIRC.csv")
...
kegg_res<-as.data.frame(kegg) #无

setwd('D:/VCAN富集分析')
write.csv(hallmark_res,"KIRC-HALLMARK_result.csv")
write.csv(go_res,"KIRC-GO_result.csv")
#write.csv(kegg_res,"KIRC-KEGG_result.csv")

##画图
GO_upanddown_part<-read.table("D:/Code/Data/FigS4/KIRC-GO_result.csv",header=T,row.names=1,sep=",")
HALLMARK_upanddown_part<-read.table("D:/Code/Data/FigS4/KIRC-HALLMARK_result.csv",header=T,row.names=1,sep=",")

CRGs_FEA_plot<-rbind.data.frame(GO_upanddown_part,HALLMARK_upanddown_part,stringsAsFactors = FALSE)
...
library(ggplot2)
Fig.S4B <- ggplot(CRGs_FEA_plot, aes(x=CRGs_FEA_plot$Description)) + 
  geom_bar(aes(y =  -log10(pvalue)/scaleFactor, fill = ONTOLOGY), stat = "identity", size = 0.8, width = 0.7) +
  scale_fill_manual(values = c("#E21F26","orangered","gold2","#2278B3")) + 
...
library(enrichplot)
Fig.S4C <- gseaplot2(go, "GO:0016477", color = "green", 
                     title='Cell Migration',
                     base_size = 15,
                     pvalue_table = T,subplots=1:2,rel_heights=c(1, .2, .6))
Fig.S4C <- gseaplot2(hallmark, "HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION", color = "green", 
