...
for(i in 1:nrow(neg_freq)){
  ...
  setwd("D:/KICH-与每个通路相关的gene/neg")
  name<-paste(neg_freq[i,1],".txt")
  write.table(path,name,sep="\t",row.names = F,col.names=T,quote=F)
}

##整理画图数据
dat<-read.table('D:/Code/Data/Fig4/KICH-与每个通路相关的差异表达基因表达谱.csv',header=T,sep=',')
datnew <- dat[,-2]
...
group2 = data.frame(Path = factor(rep(c("HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION","HALLMARK_OXIDATIVE_PHOSPHORYLATION",
                                        "HALLMARK_XENOBIOTIC_METABOLISM","HALLMARK_COAGULATION",
                                        "HALLMARK_KRAS_SIGNALING_DN","HALLMARK_IL6_JAK_STAT3_SIGNALING"), 
                                      c(8,11,15,11,15,2))))
ann_colors = list(Path = c(HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION="#2980B9", 
                           HALLMARK_OXIDATIVE_PHOSPHORYLATION="#F48FB1",
                           HALLMARK_XENOBIOTIC_METABOLISM="#FBC02D", 
                           HALLMARK_COAGULATION="#FF8F00",
                           HALLMARK_KRAS_SIGNALING_DN="#26C6DA",
                           HALLMARK_IL6_JAK_STAT3_SIGNALING="#A3E4D7"),                 
                  Sample = c(cancer="#EF5350", normal="#00897B"))
#
x <- replicate(62,rnorm(100,0,1),simplify = FALSE)
x <- setNames(x,paste0("A",seq_along(x)))
rownames(group2)<- names(x)
rownames(res2)<-names(x)

library(pheatmap)
Fig.4A <- pheatmap(res2,
                   cluster_rows = F,   
                   cluster_cols = F,               
...
exp<-read.table("D:/KICH-gene_expression.csv",header=T,row.names=1,sep=",")
...
tres<-t(res)
write.csv(tres,"D:/VCAN高低风险组分组/KICH.csv")
...
hallmark_res<-as.data.frame(hallmark) #无
go_res<-as.data.frame(go)
kegg_res<-as.data.frame(kegg) #无

setwd('D:/VCAN富集分析')
#write.csv(hallmark_res,"KICH-HALLMARK_result.csv")
write.csv(go_res,"KICH-GO_result.csv")
#write.csv(kegg_res,"KICH-KEGG_result.csv")

##画图
GO_upanddown_part<-read.table("D:/Code/Data/Fig4/KICH-GO_result.csv",header=T,row.names=1,sep=",")

CRGs_FEA_plot<-GO_upanddown_part
#CRGs_FEA_plot<-rbind.data.frame(GO_upanddown_part,KEGG_upanddown_part,HALLMARK_upanddown_part,stringsAsFactors = FALSE)
...
library(ggplot2)
Fig.4B <- ggplot(CRGs_FEA_plot, aes(x=CRGs_FEA_plot$Description)) + 
  geom_bar(aes(y =  -log10(pvalue)/scaleFactor, fill = ONTOLOGY), stat = "identity", size = 0.8, width = 0.7) +
  scale_fill_manual(values = c("#E21F26","orangered","gold2")) + 
...
library(enrichplot)
Fig.4C <- gseaplot2(go, "GO:0072073", color = "green", 
                    title='kidney epithelium development',
                    base_size = 15,
                    pvalue_table = T,subplots=1:2,rel_heights=c(1, .2, .6))
Fig.4C <- gseaplot2(go, "GO:0072562", color = "green", 
                    title='Blood microparticle',
                    base_size = 15,
                    pvalue_table = T,subplots=1:2,rel_heights=c(1, .2, .6))
Fig.4C
